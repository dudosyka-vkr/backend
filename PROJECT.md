...
│   │   │   └── TestDao.kt                     # create(), findAll(), findById(), update(), deleteById(), updateImageRoi()
│   │   ├── dto/
│   │   │   └── TestDtos.kt                    # TestResponse, TestListResponse, UpdateRoiRequest, UpdateRoiResponse, ErrorResponse
│   │   └── service/
│   │       └── TestService.kt                 # Бизнес-логика + файловый I/O
│   ├── record/
│   │   ├── controller/
│   │   │   └── RecordController.kt            # POST /records, GET /records, GET /records/{id}, GET /records/users/suggest
│   │   ├── dao/
...
│   │   │   └── RecordDao.kt                   # create(), findById(), findAll() с пагинацией и фильтром testId, suggestUsers()
│   │   ├── dto/
│   │   │   └── RecordDtos.kt                  # CreateRecordRequest, RecordDetailResponse, RecordListResponse, UserSuggestResponse
│   │   └── service/
...
│       ├── V5__add_fixation_tracking_area_to_test_images.sql  # Колонка fixation_tracking_area в test_images
│       └── V6__rename_fixation_tracking_area_to_roi.sql       # Переименование в roi
├── src/test/kotlin/org/eyetracker/
│   ├── base/
│   │   ├── DatabaseTestBase.kt                # Базовый класс для DAO-тестов (прямое подключение к postgres)
...
│   ├── test/
│   │   ├── dao/TestDaoTest.kt                 # 17 тестов TestDao
│   │   ├── service/TestServiceTest.kt         # 27 тестов TestService (MockK)
│   │   └── controller/
│   │       └── TestControllerIntegrationTest.kt  # 42 интеграционных теста
│   └── record/
│       ├── dao/RecordDaoTest.kt               # 16 тестов RecordDao
│       ├── service/RecordServiceTest.kt       # 15 тестов RecordService (MockK)
│       └── controller/
...
| V5 | Колонка `fixation_tracking_area TEXT NULLABLE` в таблице `test_images` |
| V6 | Переименование `fixation_tracking_area` → `roi` в таблице `test_images` |
...
| PATCH | `/tests/images/{id}/roi` | JWT (ADMIN+) | Обновить ROI изображения по его ID | `200 UpdateRoiResponse` или `404` |
...
```json
{
    ...
    "imageIds": [1, 2],
    "rois": [null, "{\"x\":10,\"y\":20,\"w\":100,\"h\":50}"],
    "createdAt": "2025-01-15T12:00:00Z"
}
```

### Формат UpdateRoiResponse
...
| GET | `/records/users/suggest` | JWT | Список уникальных userLogin с фильтрами | `200 UserSuggestResponse` |

Query-параметры для `GET /records`:
- `page` (Int, default 1) — номер страницы
- `pageSize` (Int, default 20, max 100) — размер страницы
- `testId` (Int?) — фильтр по ID теста
...
Query-параметры для `GET /records/users/suggest`:
- `page` (Int, default 1) — номер страницы
- `pageSize` (Int, default 20, max 100) — размер страницы
- `testId` (Int?) — фильтр по ID теста
- `from` (ISO-8601 String?) — начало временного диапазона (по startedAt)
...
174 теста, разделённых на три категории:

| Категория | Количество | Подход |
|-----------|------------|--------|
| Unit-тесты сервисов (AuthServiceTest, TestServiceTest, RecordServiceTest) | 56 | MockK для моков DAO, без БД |
| Тесты DAO (UserDaoTest, TestDaoTest, RecordDaoTest) | 42 | Прямое подключение к PostgreSQL, реальная БД |
| Интеграционные тесты (контроллеры Auth + Test + Record) | 76 | Полное приложение Ktor + реальная БД, реальные HTTP-запросы |

### Инфраструктура тестов

**DatabaseTestBase** — базовый класс для DAO-тестов:
- Подключается напрямую к PostgreSQL (`jdbc:postgresql://localhost:5432/eyetracker_test`)
- `@BeforeEach`: Flyway clean + migrate
- `@AfterEach`: TRUNCATE всех таблиц

> Требуется работающий PostgreSQL. Запустить можно через `cd deploy && docker-compose up -d postgres`.

**IntegrationTestBase** — базовый класс для интеграционных тестов:
- Подключается напрямую к PostgreSQL (`jdbc:postgresql://localhost:5432/eyetracker_integration`)
...
- `TestService`: create (успех, пустое имя, нет изображений, ошибка I/O), getAll, getById, update, delete, getCoverFile, getImageFile (все граничные случаи), updateImageRoi (успех, не найден)
- `RecordService`: create (валидация: empty items, negative duration, invalid timestamp, test not found, invalid imageId), create success, getById, getAll (пагинация, clamping, фильтр testId), suggestUsers
...
- Создание прохождений, пагинацию, фильтрацию по userLogin, testId и временному диапазону
- Обновление ROI изображения, suggest пользователей

### Запуск тестов

```bash
# Все тесты (требуется запущенный PostgreSQL — cd deploy && docker-compose up -d postgres)
make test
